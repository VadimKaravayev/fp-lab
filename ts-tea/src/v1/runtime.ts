...
import { type VNode, render } from "./h.js";
import type { Cmd } from "./cmd.js";

// A Program is the complete description of a TEA application: how to
// start, how to react to messages, and how to render state.
export type Program<Model, Msg> = {
  readonly init: () => readonly [Model, Cmd<Msg>];
  readonly update: (msg: Msg, model: Model) => readonly [Model, Cmd<Msg>];
  readonly view: (model: Model) => VNode<Msg>;
};
  ...
  const dispatch = (msg: Msg): void => {
    ...
    try {
      while (queue.length > 0) {
        ...
        if (next === undefined) break; // satisfies noUncheckedIndexedAccess
