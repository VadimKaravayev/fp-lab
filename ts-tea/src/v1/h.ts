...
export type VNode<Msg> =
  | { readonly kind: "text"; readonly text: string }
  | {
      readonly kind: "element";
      readonly tag: string;
      readonly attrs: Attrs<Msg>;
      readonly children: readonly VNode<Msg>[];
    };

export type Handler<Msg> = (event: Event) => Msg;

export type AttrValue<Msg> = string | number | boolean | Handler<Msg>;

export type Attrs<Msg> = { readonly [key: string]: AttrValue<Msg> };
...
export function text<Msg = never>(s: string): VNode<Msg> {
...
export function h<Msg>(
  tag: string,
  attrs: Attrs<Msg>,
  children: readonly (VNode<Msg> | string)[] = [],
): VNode<Msg> {
  const normalized: VNode<Msg>[] = children.map((c) =>
    typeof c === "string" ? text<Msg>(c) : c,
  );
...
type PrevSlot<Msg> = { vnode: VNode<Msg>; node: Node };
...
export function render<Msg>(
  ...
  const host = parent as HTMLElement & { [PREV]?: PrevSlot<Msg> };
...
function build<Msg>(vnode: VNode<Msg>, dispatch: (msg: Msg) => void): Node {
  if (vnode.kind === "text") {
...
function patchChildren<Msg>(
  parent: Node,
  oldChildren: readonly VNode<Msg>[],
  newChildren: readonly VNode<Msg>[],
  dispatch: (msg: Msg) => void,
): void {
...
type HandlersSlot = Record<string, ((event: Event) => unknown) | undefined>;
type HandlerHost = HTMLElement & { [HANDLERS]?: HandlersSlot };
    ...
    if (slot[eventName] === undefined) {
      ...
      el.addEventListener(eventName, (event) => {
        ...
        if (current !== undefined) {
          dispatch(current(event) as Msg);
        }
      });
    }
    slot[eventName] = value as (e: Event) => unknown;
    return;
  }
...
function removeAttr<Msg>(
  el: HTMLElement,
  key: string,
  oldValue: AttrValue<Msg>,
): void {
    ...
    const reset: unknown =
      typeof oldValue === "boolean"
        ...
        : typeof oldValue === "number"
          ? 0
          : "";
    (el as unknown as Record<string, unknown>)[key] = reset;
  } else {
